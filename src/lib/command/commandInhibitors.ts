 ...
 */
const userPermissionInhibitor = function(command, data: {member: GuildMember, guild: Guild}) {
...
const botPermissionInhibitor = function(command, guild: Guild) {
    ...
    if (requiredPermissions.some((permission) => !botPermissions.includes(permission)) &&
...
const isDisabled = async function(command, guild: Guild) {
    ...
    if (guildConfig[0].disabledCommands.includes(command.name) || globalConfig[0].disabledCommands.includes(command.name)) return false;
...
export { isDevOnly, isOnCooldown, isDisabled, botPermissionInhibitor, userPermissionInhibitor };
