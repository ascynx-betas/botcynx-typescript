...
export default new Event('messageCreate', async (message) => {
    ...
    if(message.author.bot ||
        ...
        const su = info.su;
        if (!su.includes(message.author.id) && message.author.id != process.env.developerId) return; //message commands can only be used by super-users or the developer
        if (command.devonly === true && message.author.id != process.env.developerId) return; //In message commands, devonly means that it can only be used by the set developer.
