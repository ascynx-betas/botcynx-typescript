...
export default new Event('guildMemberUpdate', async(oldMember, newMember) => {
    ...
    if (!botPermissions.includes('MANAGE_ROLES') &&
        ...
        if (TriggerSimilaritiesOldmember.breakingcount < TriggerSimilaritiesNewmember.breakingcount ||
              ...
              if (TriggerSimilaritiesOldmember.breakingcount > TriggerSimilaritiesNewmember.breakingcount ||
                (typeof TriggerSimilaritiesNewmember.breakingcount === "undefined") &&
                 TriggerSimilaritiesOldmember.breakingcount > 0) {
                    ...
                    let newMemberRemovables = newMemberRoles.filter((x) => removable.includes(x));
