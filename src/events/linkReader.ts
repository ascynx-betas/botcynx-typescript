...
export default new Event('messageCreate', async (message) => {
    ...
    if (!botPermissions.includes('MANAGE_WEBHOOKS') &&
        ...
        if (result == false) return message.react("❌"); //link contains a non-id
        const source = await (botcynx.channels.cache.get(fields[3]) as GuildTextBasedChannel).messages.fetch(fields[4]).catch(() => {return message.react("📵")} /**the message does not exist */)
        ...
        let sourceGuildMember = (source as Message<boolean>).guild.members.cache.get((source as Message<boolean>).author.id);
        if (typeof sourceGuildMember !== "undefined") {
            ...
            avatarURL = (source as Message<boolean>).author.displayAvatarURL({ dynamic: true });
        } else {
        ...
        let attachmentsUrls: any = (source as Message<boolean>).attachments.map(a => a.url);
        let embeds = (source as Message<boolean>).embeds.filter(embed => embed.type === "rich") //! using deprecated property, might fail at any time

        if (isThread == true) {
            webhook = await (message.channel as ThreadChannel).parent.fetchWebhooks();
        } else webhook = await (message.channel as ThreadChannel).parent.fetchWebhooks();
        webhook = webhook.filter(webhook => webhook.owner.id === botcynx.user.id);

        if (typeof webhook === "undefined" || webhook.size == 0) {webhook = await (message.channel as TextChannel).createWebhook("Botcynx link reader", {
