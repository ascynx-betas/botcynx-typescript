  ...
  run: async ({ client, interaction }) => {
    ...
    if (!userInfo?.length)
      return interaction.followUp({
        content: `user isn't verified, please tell them to use the /verify command`,
        ephemeral: true,
      });
    const uuid: string = info.minecraftuuid;
    if (typeof uuid === "undefined")
      return interaction.followUp({
        content: `couldn't fetch uuid`,
        ephemeral: true,
      });
    ...
    if (info.labels.length > 0) {
      ...
      return interaction.followUp({
        content: `${specificity || ""} ${
          user.tag
        }'s username is \`\`${username}\`\` their uuid is ${uuid} and they have the label(s) ${labellist.join(
          ", "
        )}`,
        ephemeral: true,
      });
