...
export default new Command({
    ...
    run: async ({message, client, args}) => {
        ...
        if (!member) return message.reply({content: `this user isn't in this guild`});
        
        const channels = message.guild.channels.cache.filter(c => c.type === "GUILD_TEXT");

        channels.forEach(async (channel) => {
            let messages = await (channel as TextChannel).messages.fetch({limit:50});
            messages = messages.filter(m => m.author.id == member.id);
            messages.forEach((m) => m.delete().catch(null));
        })
        member.kick().then((member) => m.reply({content: `kicked ${member} and deleted messages / threads`}));
