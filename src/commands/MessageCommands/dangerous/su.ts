...
export default new Command({
    ...
    run: async({ message, client, args }) => {
        ...
        if (!user) return message.reply({content: `please specify who you want to add or remove`});
        ...
        if (Id.length != 18) return message.channel.send({content: `the Id doesn't follow standards for an id`});
        ...
        if (!flags || flags == "-l") {
            if (su.includes(Id)) {
                ...
                message.reply({content: `successfully removed <@${Id}> from ${message.guild.name}'s super user list`});
                ...
                message.reply({content: `successfully added <@${Id}> to ${message.guild.name}'s super user list`});
