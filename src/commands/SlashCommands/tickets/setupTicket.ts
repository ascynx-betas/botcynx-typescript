        ...
        const buttonRow = new MessageActionRow().addComponents(
            new MessageButton()
                ...
                const embed = new MessageEmbed()
                    ...
                    let sent: Message;
                    await (setupChannel as GuildTextBasedChannel).send({embeds: [embed], components: [buttonRow]}).then((message) => {
                        interaction.followUp({content: `success, created a new ticket message in ${setupChannel}`});
                        sent = message;
                    }).catch(() => interaction.followUp({content: `I don't have permission to send a message in the specified channel`}));
