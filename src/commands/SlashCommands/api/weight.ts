...
import { extractWeight, getFatterProfile, getSpecifiedProfile } from "../../../personal-modules/senither";
...
export default new slashCommand({
    ...
    run: async({ interaction }) => {
        ...
        if (!username) {
            ...
            if (!uuid) return interaction.followUp({content: `please enter the username of the person you want to see the weight of`});
        ...
        if (typeof uuid == "undefined") {
            ...
            console.log(uuid)
            if (uuid == null) return interaction.followUp({content: `please enter the username of the person you want to see the weight of`});

        } else {

            const data = await getPlayerByUuid(uuid).catch(() => null);

            if (!data) return interaction.followUp({content: `error while trying to fetch username`});
        ...
        if (profile) {
            ...
            profile != "peach"
                ...
                const embed = new MessageEmbed()
                    .setDescription(`${profile} doesn't seem to match any possible profile names, if you feel like that's an error, please contact the developer`)
        ...
        const result = await extractWeight(data);
            if (result == null) {
                const embed = new MessageEmbed()
                    ...
                    .setAuthor({name: `Error 404: not found`})
                    .setThumbnail(`https://http.cat/404`);
                    return interaction.followUp({content: `https://sky.shiiyu.moe/resources/video/enable-api.webm`,embeds: [embed]});
            }
    ...
    const embed = new MessageEmbed()
      ...
      const buttonRow = new MessageActionRow().addComponents(
          new MessageButton()
            ...
            interaction.followUp({embeds: [embed], components: [buttonRow]}).catch(() => null);
    }
})