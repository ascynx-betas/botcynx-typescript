...
export default new slashCommand({
    ...
    run: async({ interaction, client }) => {
        ...
        if (type === "bypass") {
            ...
            const roleId = role.id;
            if (guildConfig.trigger.includes(roleId)) return interaction.followUp({
            ...
            configModel.updateOne(
                ...
                function (err: any) {
                    if (err) return interaction.followUp({content: `there was an error while trying to update the configuration, please try again later`});
                });

                interaction.followUp({content: `${role} has successfully been added to ${type}`, allowedMentions: {parse:[]}});
        } else if (type === "removable") {
            const roleId = role.id;
            if (guildConfig.removable.includes(roleId)) return interaction.followUp({content: `removable already contains this role, if you want to remove it, please use the /delconfig command`});
            ...
            if (guildConfig.logchannel == `${logChannel}`) return interaction.followUp({content: `the current Logging channel is the same as the one you're trying to set it to`});
            ...
            configModel.updateOne(
                ...
                interaction.followUp({content: `the new Logging channel is ${logChannel}, modifications after this one will also be logged there`});
        } else if (type == "blockChannel") {
            const channelId = channel.id;

            if (guildConfig.blocked.includes(channelId)) return interaction.followUp({content: `${channel} is already blocked, if you want to unblock it, please use the /delconfig command`});
            ...
            configModel.updateOne(
                ...
                interaction.followUp({content: `${channel} is now blocked, any person that tries to use the bot's link reader on a link that leads to that channel, will be ignored`});
