...
export default new slashCommand({
    ...
    run: async({ interaction }) => {
        ...
        if (removable.length < 1) return interaction.followUp({content: `there's no removable roles, set some then come back later`});
        ...
        removable.forEach(function (removable) {
            const members = guild.members.cache.filter((m) => m.roles.cache.has(removable)); // get all members that have the selected role

            members.forEach(function(member) {
                let roles = guild.members.cache.get(member.id).roles.cache.map((r) => r.id);
                ...
                let has;
                    ...
                    if (has.success === false) {
                        if (isTestRun == false) {
                            guild.members.cache.get(member.id).roles.remove(removable).then(() => (affectedMembers.push(String(member))));
            ...
            if (index >= r.length-1) {
                ...
                if (isTestRun == true) description = `Test run\n **Affected members**:\n${affectedString}`;
                if (isTestRun == false) description = `**Affected members**:\n${affectedString}`;
            ...
            if (index >= r.length-1) return interaction.followUp({content: `the command seems to have run into an error`});
